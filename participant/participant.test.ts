...
describe('participant lifecycle', () => {
  ...
  test('preserves worker startup failure when cleanup also fails', async () => {
    const startFailure: WorkerStartFailure = {
      _tag: 'worker-start-failed',
      reason: FailureReason('worker did not become ready'),
    }
    const shutdownFailure: WorkerShutdownFailure = {
      _tag: 'worker-shutdown-failed',
      reason: FailureReason('worker cleanup failed'),
    }
    const participant = createParticipant({
      config: validConfig,
      identitySource: {
      ...
      worker: {
        start: Effect.fail(startFailure),
        shutdown: Effect.fail(shutdownFailure),
      },
    })

    await expectFailure(participant.start(), startFailure)
  })
