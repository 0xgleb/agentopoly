...
The first arbitration vertical binds the original job ID, terms hash, artifact hash, failed verification hash, bounded disputing statement, and submission time into one replay-protected bundle. A dispute bundle or ruling older than 60 seconds is refused. A ruling for that bundle is a separately signed delivery by an arbitrator under a separately signed arbitration agreement. The ruling creates an arbitration settlement intent only after a passed verification binds that distinct arbitration job, agreement, ruling artifact, and verification evidence. The intent fixes the arbitrator's agreed atomic amount, destination, and network. A schema-version-2 receipt event retains its live or recorded-fixture provenance and is accepted as live payment evidence only for that arbitration job when every receipt field and the locally allocated attempt ID and authorization key match exactly. Replaying the same event returns its recorded result without appending another event; conflicting or fixture-derived evidence cannot claim live payment. It never authorizes, releases, pays, or otherwise mutates the original provider payment. Only the original job's local policy may later make an original-payment decision.
