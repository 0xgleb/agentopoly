...
A narrow local gateway accepts only the decoded closed command union and defaults to typed unavailable without starting a process. An operator may inject the gateway implementation that launches `wdk-mcp` over private inherited stdio and maps fixed typed calls to `get_address`, `get_balance`, `get_history`, and `send_token`; no model, peer, browser command, provider process, or general MCP client may receive that capability. The repository proves the injected boundary with captured response fixtures and spies only; it does not claim that a funded wallet process was started or that live broadcast is safe. A compromised same-user process may still reach the human-unlocked WDK daemon directly, which remains a disclosed residual risk. The human creates and unlocks the dedicated tiny-balance wallet, and Agentopoly never handles its passphrase or seed.
...
- **WDK:** the selected boundary assigns address, balance, history, preview, broadcast, and settlement observation to an injected, pinned operator-local `wdk-mcp` sidecar gateway and wallet daemon; the default gateway is unavailable, the Pear worker has no wallet capability, and repository tests start no wallet process.
