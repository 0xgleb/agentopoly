...
Either party may open a dispute according to the signed arbitration policy. When verification fails, the buyer withholds the original provider payment before opening the dispute. A dispute bundle contains only evidence already bound to the job plus the disputing statement. The first arbitration vertical binds the original job ID, terms hash, artifact hash, failed verification hash, and bounded disputing statement into one replay-protected bundle. A ruling for that bundle is a separately signed delivery by an arbitrator under a separately signed arbitration agreement; it creates only that arbitration job's guarded settlement intent and receipt path. It never authorizes, releases, pays, or otherwise mutates the original provider payment. Only the original job's local policy may later make an original-payment decision.
