...
A payment receipt binds the job, authorization key, attempt ID, terms hash, verification hash, transaction hash, source and destination, atomic amount, asset and token contract, network, estimated and observed native fee, policy revision, broadcast time, and observed settlement state. Receipt creation never upgrades an unconfirmed observation to final settlement. A versioned `receipt.recorded` browser projection accepts only that complete locally recorded witness set. It exposes only bounded job, integer atomic USDt amount, asset, network, broadcast time, policy revision, and settlement state; it never exposes destination, source, authorization/attempt IDs, or full hashes, and it cannot authorize a wallet action. Bounded legacy receipt envelopes remain ignored; malformed or conflicting versioned receipts fail closed.
